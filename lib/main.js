...
var mouseWorker;
var mouseWorkerAttached = false;

function injectMM(tab){
  if(tab.url.match(/facebook\.com/g)){
    if(!mouseWorkerAttached){
      console.log('we need to attach');
      mouseWorkerAttached = true;
    } else {
      console.log('we is attached');
    }
  }
}

tabs.on('activate', function(tab){
  injectMM(tab, 'activate');
});
  ...
  if(tabs.activeTab.url != undefined){
    if(tabs.activeTab.url.match(/facebook\.com/g)){
      ...
      if(timeSpent > maxTime && warningAttached == 0){
        ...
        worker.port.on('detectMouse', function(detected){
          console.log(1);
        });
